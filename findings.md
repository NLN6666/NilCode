...
- scoped format commit 后的 NilCode CI run `31863597651` 已终态：Windows Process Regression、Migration Lineage、Release Smoke 与 PR policy jobs 均通过；主 job 再次只在同一个未修改 base 文档的 `fmt:check` 失败。PR-owned 26 文件 targeted oxfmt check 已通过，因此没有修改 baseline 文档来伪造绿色 CI，也没有运行本地整仓 `bun fmt` / `bun lint` / `bun typecheck`。
- OMP Draft PR #8609 的官方 `CI` run `31863300837` 与 `OMP Nix` run `31863300824` 实时可见，但二者均为 `completed/action_required`、0 秒、无 job/check；workflow 本身 active 且监听 `pull_request`，同时间其他 PR 正常通过。结论是 fork workflow 等待 upstream maintainer approval，无法由 fork 作者启动；formal native matrix 因此未执行，不能声称 upstream CI/native gate 成功。
- 第一次 `bun pm pack` 同时传 `--destination` 与 `--filename`，Bun 在 prepack 成功后拒绝参数组合且未留下 tarball；改为只传 destination 后成功。从 `591eaaa21e` 得到 1648-entry、10425363-byte tarball，SHA-256 `06338edc275b5534f1409442cf6942882cee7efaac3afc7fe06bb34ad6d617f1`。包内没有 `.node`，隔离安装解析的 Windows baseline native leaf 来自官方 17.3.4 package，hash 为 `966091fdf8d50a26024226b1bd5b93ba702378202a195b7029fdba7a92e93138`；这不是本地 formal native build provenance。
- stock 17.3.4 的真实 ACP initialize/auth/new/close 成功，而 `_omp/capabilities` 返回 `RequestError -32603`；patched artifact 对同一隔离 prefix/profile/cwd 返回 schema v1，并完成 Advisor status/drain、Auto-Learn status/drain、Memory status/stats/diagnose/enqueue/clear-challenge、Launch empty list。Memory clear 未 confirm，Launch 未 stop/restart；generation 稳定、sequence 1..11 单调。回滚 stock 后普通 ACP 再次成功且 typed 回到 `-32603`。
- 首次 rollback smoke 误把脚本要求的三个命令行参数写成环境变量，脚本在启动 OMP 前即 fail-fast；更正为 explicit binary/profile/cwd args 后通过。隔离 Synara 的第一次实际启动使用 Bun，数据库迁移后按项目自身 Windows guard 报 `Bun PTY terminal support is unavailable` 并干净退出；按错误提示改用已构建的 Node CLI 后，隔离 8094 listener 与 `/health` readiness 全部正常。
- 临时 WS probe 先后遇到根 workspace `ws` 路径错误、Bun/`ws` 101 handshake 兼容问题，以及手写 Effect RPC frame 无响应；最终只把已成功的 HTTP negotiation/WebSocket upgrade 记为 transport 辅助证据，不将未完成 framing 伪报为产品或 provider 失败。Synara 已真实加载隔离 `settings.json`，其中 OMP `binaryPath` 指向 patched prefix；同一 binary 的 typed v1 能力由独立真实 ACP smoke 闭合。
- Synara 隔离实例精确 root PID 及 1 个 descendant 已在 command-line/root guard 后终止，8094/10054 listener、匹配隔离 root 的 OMP/Synara process 均为零。patched→stock rollback 后又从隔离 prefix 卸载；Bun 留下一枚孤立 shim，因此按 exact-root guard 清理整个 adoption root。全局 `omp.exe` 仍是 17.3.4，SHA-256 `59b379b53354da72d2c5262119fe70c44b4e473826ebbaa94d47a2d58a359b1a`，与执行前一致。
- adoption root 清理的 PowerShell `Remove-Item` 与 Git Bash `rm -rf` 均被执行策略在进程创建前拒绝，没有执行；改用 guarded `.NET Directory.Delete` 后又因 Bun cache 只读项 `7.29.7@@@1` 中止并留下部分目录。最终在同一精确 root 上递归清除只读属性，再用 `.NET Directory.Delete` 完成；`ADOPTION_ROOT_EXISTS=False`，独立 artifact root 仍存在且 tarball SHA-256 复核不变。
