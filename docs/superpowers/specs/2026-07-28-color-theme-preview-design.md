...
注入内容说明两种围栏格式，并明确引导：**优先 ` ```html theme `，只在用户要的是裸 token 列表而非渲染效果时才用 ` ```theme `**。

> **2026-07-29 修订**：初版引导相反（优先 ` ```theme `）。实测发现用户想要的是「看到配色应用在真实界面上」，色板行列表满足不了这个诉求，故翻转优先级。代价是 HTML 路径的「采用」较弱（见第 5 节），这是明知的取舍。优先级顺序由 `colorPreviewPromptInjection.test.ts` 断言锁定。
...
这一路径的「采用」明显弱于结构化路径。初版据此把 ` ```theme ` 定为主路径，但 2026-07-29 修订翻转了优先级（见 3.2 节）：用户要的是看见配色应用在界面上，这个诉求压过了采用消息的精确度。` ```theme ` 保留为次要路径，用户想要精确 token 列表时仍可用。
