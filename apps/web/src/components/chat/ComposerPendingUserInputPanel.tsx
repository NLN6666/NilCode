...
function ComposerPendingUserInputCard({
  ...
  const copy = useMessages().composer.pendingInput;
  ...
  return (
    <div className={cn(COMPOSER_INPUT_SURFACE_CLASS_NAME, "overflow-hidden px-3.5 py-3")}>
        ...
        {showNavigation ? (
          <div className="flex shrink-0 items-center gap-0.5 pt-px text-muted-foreground/70">
            ...
            <span className="px-0.5 text-[11px] tabular-nums">
              {copy.progress(progress.questionIndex + 1, questionCount)}
            </span>
            <button
              type="button"
      ...
      {activeQuestion.multiSelect ? (
        <p className="mt-1 text-[11px] text-muted-foreground/55">{copy.selectMultiple}</p>
      ) : null}
