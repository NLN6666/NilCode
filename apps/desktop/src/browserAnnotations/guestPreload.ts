...
const GUEST_COPY = {
  en: {
    ...
    noteLabel: "Annotation comment",
    submitLabel: "Save annotation",
    submitTitle: "Save annotation (Enter)",
  },
  "zh-CN": {
    dialogLabel: "标注元素",
...
function applyGuestCopy(locale: BrowserAnnotationLocale): void {
  ...
  textarea?.setAttribute("aria-label", copy.noteLabel);
  // The submit control is an icon button in this shell, so its copy lives in the
  // accessible name and tooltip — writing textContent would drop the SVG.
  submitButton?.setAttribute("aria-label", copy.submitLabel);
  submitButton?.setAttribute("title", copy.submitTitle);
}
...
const OVERLAY_MARKUP = `
  ...
  <section class="popover" role="dialog" hidden>
    ...
    <textarea rows="1" maxlength="${GUEST_ANNOTATION_MAX_COMMENT_LENGTH}"></textarea>
    <button type="button">
