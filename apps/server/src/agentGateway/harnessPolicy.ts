...
export const SYNARA_HARNESS_POLICY_VERSION = "2026-08-08.2";
...
export function renderSynaraHarnessPolicy(capabilities: SynaraHarnessCapabilities): string {
  const controlPolicy = capabilities.gatewayControlAvailable
    ? [
        ...
        "Any process that must outlive this turn — a game server, dev server, database, or anything else the user will still be using afterwards — must be started with the synara_*_daemon tools. Never background it from a shell instead (no `&`, `nohup`, `setsid`, `start /b`, PowerShell `Start-Process` or `Start-Job`, or a run-in-background shell flag): a shell-backgrounded process is a child of this session's process tree and is killed when the session tears down, and it is invisible in Synara's background services panel, so the user can neither watch its log nor stop it. Declare a `ready` condition on synara_start_daemon rather than polling, drive a stateful server with synara_send_daemon_input (send its own shutdown command before reaching for synara_stop_daemon), and set `detached: true` only when the process must also survive Synara itself exiting.",
...
export interface SynaraHarnessPolicyDeliveryState {
  /**
   * Policy version this session has already seen.
   *
   * Deliberately a version rather than a "delivered" flag: a boolean latches, so a session
   * that was already open when the policy changed would keep the superseded rules for its
   * whole life and never learn a newly added constraint.
   */
...
/** Return the private host-context block once per policy version for one provider session. */
export function takeSynaraHarnessPolicyForSession(
  state: SynaraHarnessPolicyDeliveryState,
  capabilities: SynaraHarnessCapabilities,
): string | null {
  if (state.deliveredHarnessPolicyVersion === SYNARA_HARNESS_POLICY_VERSION) return null;
  state.deliveredHarnessPolicyVersion = SYNARA_HARNESS_POLICY_VERSION;
  return [
    "<synara_host_context>",
